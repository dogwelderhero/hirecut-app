  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-10">
      <div className="flex items-end justify-between">
        <div>
          <h1 className="font-display text-2xl tracking-tight text-foreground">Workers</h1>
      ...
      {jobs.length === 0 ? (
        <div className="mt-8 rounded-2xl border border-dashed border-border bg-card/30 px-6 py-12 text-center text-sm text-muted-foreground">
          No workers yet. Hit <span className="text-foreground">Evaluate</span> on an inbox posting to spin one up.
        </div>
      ) : (
        <ul className="mt-6 divide-y divide-border overflow-hidden rounded-2xl border border-border bg-card/40">
          {jobs.map((j) => {
            const tone = pillTone(j);
            return (
              <li key={j.id}>
                <Link href={`/jobs/${j.id}`} className="flex items-center gap-3 px-4 py-3 transition-colors hover:bg-accent">
                  {j.status === "running" ? (
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {(j.subtitle || j.result?.summary) && (
                      <div className="truncate text-xs text-muted-foreground">{j.result?.summary || j.subtitle}</div>
                    )}
                  </div>
                  {j.result?.score != null && (
                  ...
                  <span className="hidden shrink-0 text-xs capitalize text-muted-foreground sm:block">{j.status}</span>
                </Link>
              </li>
            );
          })}
