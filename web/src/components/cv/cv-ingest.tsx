...
function cliId(): string | null {
  try {
    return JSON.parse(localStorage.getItem("hirecute:config") || "{}").cliId || null;
  } catch {
    return null;
  }
}
    ...
    return (
      <div className="rounded-2xl border border-border bg-card/60 p-6 backdrop-blur-sm">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <span className={`${GeistSans.className} text-lg text-foreground`}>{trace || "Reading your CV…"}</span>
        </div>
  ...
  return (
    <div className="rounded-2xl border border-border bg-card/60 p-4 backdrop-blur-sm md:p-5">
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2">
        ...
        <h3 className={`${GeistSans.className} text-lg text-foreground`}>Here&apos;s your CV — review and save</h3>
        {readiness && (
          <span
            className={cn(
