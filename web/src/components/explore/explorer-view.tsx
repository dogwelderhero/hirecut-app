...
export function ExplorerView({
  ...
  useEffect(() => {
    try {
      const id = JSON.parse(localStorage.getItem("hirecute:config") || "{}").cliId || null;
    ...
    <div className="mx-auto max-w-5xl px-5 py-8 md:px-8">
      <header className="mb-6">
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-2.5">
            ...
            <h1 className={`${GeistSans.className} text-3xl text-foreground`}>Explore</h1>
      ...
      {!rootExists && (
        <div className="mb-5 rounded-lg border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-sm text-amber-700 dark:text-amber-300">
          Your hirecute home isn’t set up yet — discovery needs a checkout with a profile to seed from.
        </div>
      )}
...
function EmptyState({ tone, title, body, note, onRerun, rerunLabel }: { tone: "good" | "loose"; title: string; body: string; note?: string; onRerun: () => void; rerunLabel: string }) {
  return (
    <div className="rounded-2xl border border-border bg-card/30 px-6 py-12 text-center">
      ...
      <h2 className={`${GeistSans.className} mt-4 text-2xl text-foreground`}>{title}</h2>
  ...
  if (scannerMissing) {
    return (
      <div className="rounded-2xl border border-border bg-card/30 px-6 py-10 text-center">
        ...
        <h2 className={`${GeistSans.className} mt-4 text-2xl text-foreground`}>Discovery needs the full toolkit</h2>
        <p className="mx-auto mt-1.5 max-w-md text-sm text-muted-foreground">
          Your hirecute home looks data-only or is on an older version. The free scanner ships with a complete checkout —
          update hirecute, or paste a job URL on the pipeline to evaluate it directly.
        </p>
...
function BlockedCard() {
  return (
    <div className="rounded-2xl border border-border bg-card/30 px-6 py-12 text-center">
      ...
      <h2 className={`${GeistSans.className} mt-4 text-2xl text-foreground`}>AI search needs a CLI</h2>
      <p className="mx-auto mt-1.5 max-w-md text-sm text-muted-foreground">
