...
export function ApplyButton({ n, url, company, pdfReady }: { n: string; url?: string; company: string; pdfReady: boolean }) {
  ...
  if (!ready) {
    return (
      <button
        ...
        className="inline-flex cursor-not-allowed items-center justify-center gap-1.5 rounded-full border border-border bg-card/40 px-3.5 py-1 text-xs font-medium text-muted-foreground max-sm:min-h-[44px]"
      >
  ...
  return (
    <button
      ...
      className="inline-flex items-center justify-center gap-1.5 rounded-full bg-primary px-3.5 py-1 text-xs font-medium text-primary-foreground shadow-sm transition-colors hover:bg-primary/90 max-sm:min-h-[44px]"
