...
const TONE: Record<string, { dot: string; label: string; chip: string }> = {
  ...
  skipped: { dot: "bg-zinc-400", label: "no ATS", chip: "bg-accent text-muted-foreground" },
};
    ...
    <div>
      <div className="flex items-center gap-3">
        <button
          onClick={check}
          disabled={loading}
          className="inline-flex items-center gap-2 rounded-full bg-primary px-4 py-2 text-sm font-medium text-primary-foreground transition-colors hover:bg-primary/90 disabled:opacity-50 max-sm:min-h-[44px]"
        >
          {loading ? <Loader2 className="size-4 animate-spin" /> : <Radar className="size-4" />}
          Check portal health
        </button>
        {loading && <span className="text-xs text-muted-foreground">Probing each company&apos;s ATS… (~30–60s)</span>}
      </div>

      {res && !res.available && (
        <p className="mt-4 rounded-xl border border-dashed border-border bg-card/30 p-4 text-sm text-muted-foreground">
          <code className="text-foreground">verify-portals.mjs</code> not found — this needs a complete hirecute
            ...
            {sorted.map((c) => {
              ...
              return (
                <li key={c.name} className="flex items-center gap-3 px-4 py-2.5">
                  ...
                  <span className="truncate font-mono text-xs text-muted-foreground">{c.detail}</span>
...
function FixAffordance({ company, job, onFix }: { company: string; job?: Job; onFix: () => void }) {
  if (job?.status === "running")
    return (
      <Link href={`/jobs/${job.id}`} className="inline-flex items-center gap-1 text-xs font-medium text-primary">
  ...
  return (
    <button
      ...
      className="inline-flex items-center gap-1 rounded-md border border-border px-2 py-1 text-xs text-muted-foreground transition-colors hover:border-primary/40 hover:text-primary"
    >
