/**
 * Fixture data — plain ESM.
 *
 * `.mjs` because the journey WORKER imports it, and a child process should not
 * depend on type stripping to read fixture rows. `fixtures.ts` re-exports this
 * with types for the browser and the tests, so there is still one source.
 *
 * Constraints (see fixtures.ts for the full rationale): nothing here is the
 * prototype's Alex Morgan profile, no score is 98%, no recruiter address is
 * invented, and unknown fields stay unknown.
 */


export const FIXTURE_RUN_ID = "sample-run";
/** A fixture journey is a sample run. It never counts as a live conversion. */
export const FIXTURE_ORIGIN = "sample";

const FETCHED_AT = "2026-09-25T09:00:00.000Z";

function source(provider, slug, jobId) {
  const host =
    provider === "greenhouse"
...
export const FIXTURE_REFINEMENT_CHANGES = [
  {
    id: "chg-1",
    kind: "clarity",
 ...
 */
export function fixtureApplications() {
...
export function fixtureLetterText(company, title) {
  return [
